...
const langs = [
  'en', 'es', 'fr', 'de', 'pt', 'pl', 'it',
  'nl', 'ru', 'ja', 'zh', 'ko', 'ar', 'hi', 'tr', 'sv', 'cs', 'sk', 'uk', 'ro', 'el', 'id', 'th', 'vi', 'hu',
];

const OG_LOCALE = {
  en: 'en_US', es: 'es_ES', fr: 'fr_FR', de: 'de_DE', pt: 'pt_PT', pl: 'pl_PL', it: 'it_IT',
  nl: 'nl_NL', ru: 'ru_RU', ja: 'ja_JP', zh: 'zh_CN', ko: 'ko_KR', ar: 'ar_AR', hi: 'hi_IN',
  tr: 'tr_TR', sv: 'sv_SE', cs: 'cs_CZ', sk: 'sk_SK', uk: 'uk_UA', ro: 'ro_RO', el: 'el_GR',
  id: 'id_ID', th: 'th_TH', vi: 'vi_VN', hu: 'hu_HU',
};
...
function setLocaleSeo(html, lang) {
  const base = 'https://mahjongboss.com';
  const baseUrl = lang === 'en' ? base + '/' : base + '/' + lang + '/';
  const ogLocale = OG_LOCALE[lang] || 'en_US';

  html = html.replace(
    /<meta property="og:url" content="https:\/\/mahjongboss\.com\/">/,
    '<meta property="og:url" content="' + baseUrl + '">'
  );
  html = html.replace(
    /<meta name="twitter:url" content="https:\/\/mahjongboss\.com\/">/,
  ...
  html = html.replace(
    /"item": "https:\/\/mahjongboss\.com\/"/g,
    '"item": "' + baseUrl + '"'
  );
  html = html.replace(
    /"url": "https:\/\/mahjongboss\.com\/"/g,
    '"url": "' + baseUrl + '"'
  );
  return html;
}

function buildStringsJs() {
  const base = translations.en;
  const out = {};
  for (const lang of langs) {
    const t = translations[lang] || base;
    out[lang] = {
      layouts: t.layouts || base.layouts,
      ui: { ...base.ui, ...t.ui, levelComplete: t.ui?.levelComplete || base.ui?.levelComplete || 'Level complete!' },
      toasts: t.toasts || base.toasts,
      win: t.win || base.win || { cheer: base.win?.cheer || 'You cleared all the tiles!', shareLabel: base.win?.shareLabel || 'Share your score:' },
    };
  }
  const js = '/* Client-side translations - generated by scripts/build-i18n.js */\nwindow.TRANSLATIONS = ' +
  ...
  fs.writeFileSync(path.join(publicDir, 'i18n', 'strings.js'), js, 'utf8');
  console.log('Generated i18n/strings.js');
}
...
function main() {
  ...
  // Always refresh hreflang on English index so new languages are listed
  let enHtml = fs.readFileSync(path.join(publicDir, 'index.html'), 'utf8');
  enHtml = setHreflang(enHtml, 'en');
  if (!enHtml.includes('data-lang="en"')) {
